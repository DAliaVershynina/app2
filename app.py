...
app = Flask(__name__)
app.secret_key = 'tajne_kluc_slova'  # bezpečnostný kľúč pre session
...
def index():
    # Ak je to prvé načítanie stránky → začni od koreňa stromu
    if "node" not in session:
        session["node"] = tree
        session["answers"] = []  # zoznam odpovedí používateľa
    ...
    if request.method == "POST":
        ...
        session["answers"].append({
            "question": node["question"],  # plný text otázky
            "odpoved": answer_label  # Áno/Nie alebo číselná hodnota
        })
